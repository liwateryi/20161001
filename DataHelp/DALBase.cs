...
namespace DataHelp
{
    public class DALBase
    {
        ...
        public MySqlHelp mySqlHelp = new MySqlHelp();
        ...
        public virtual void SelectForID(ModelBase model)
        {
            ...
            manager.Add("@" + keyName, keyValue);
            using (SqlDataReader read = sqlHelp.ExecuteReader(manager))
            {
                if (read.Read())
                {
                    //取出实体类中所有的属性名
        ...
        protected internal virtual List<T> Select<T>(SqlManager manager) where T : ModelBase
        {
            Type type = typeof(T);
            using (SqlDataReader read = sqlHelp.ExecuteReader(manager))
            {
        ...
        {
            Type type = typeof(T);
            using (MySqlDataReader read = mySqlHelp.ExecuteReader(manager))
            {
                List<T> list = new List<T>();
                while (read.Read())
                {
